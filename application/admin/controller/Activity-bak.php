...
class Activity extends Base {
	public function test() {
		...
		for ($i = 0; $i < count($list); $i++) {
			if ($list[$i]['execution_time']) {
				$newData[$n] = array(
					'activity_id' => $list[$i]['id'],
					'name' => $list[$i]['execution_time'],
				);
				$n++;
			}
		}
		$period->saveAll($newData);
	}
	...
	public function index() {
		...
		if ($tags) {
			if ($keyword) {
				if ($type) {
					$list = $activity::hasWhere('tags', ['tags_id' => $tags])->where(['shop_id' => $type, 'title' => ['like', '%' . $keyword . '%']])->order('hot desc, id desc')->paginate(10);
				} else {
					$list = $activity::hasWhere('tags', ['tags_id' => $tags])->where(['element' => 0, 'title' => ['like', '%' . $keyword . '%']])->order('hot desc,id desc')->paginate(10);
				}
			} else {
				if ($type) {
			...
			if ($keyword) {
				if ($type) {
					$list = $activity->where(['shop_id' => $type, 'title' => ['like', '%' . $keyword . '%']])->order('hot desc,id desc')->paginate(10);
				} else {
					$list = $activity->where(['element' => 0, 'title' => ['like', '%' . $keyword . '%']])->order('hot desc,id desc')->paginate(10);
				}
			} else {
				if ($type) {
					$list = $activity->where(['shop_id' => $type])->order('hot desc, update_time desc')->paginate(10);
				} else {
					$list = $activity->where($map)->order('hot desc,id desc')->paginate(10);
				}
			}
		}
